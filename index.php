<!doctype html>
<html class="no-js"  lang="en">
	<head>
		<!-- META DATA -->
		<meta charset="utf-8">
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<!-- The above 3 meta tags *must* come first in the head; any other head content must come *after* these tags -->

		<!--font-family-->
		<link href="<?php echo base_url('https://fonts.googleapis.com/css?family=Rufina:400,700" rel="stylesheet"')?>"/>

		<link href="<?php echo base_url('https://fonts.googleapis.com/css?family=Poppins:100,200,300,400,500,600,700,800,900" rel="stylesheet" ')?>"/>

		<!-- TITLE OF SITE -->
		<title>Travel</title>

		<!-- favicon img -->
		<link rel="shortcut icon" href="<?php echo base_url('assets/logo/favicon.png')?>"/>

		<!--font-awesome.min.css-->
		<link rel="stylesheet" href="<?php echo base_url('assets/css/font-awesome.min.css')?>"/>

		<!--animate.css-->
		<link rel="stylesheet" href="<?php echo base_url('assets/css/animate.css')?>"/>

		<!--hover.css-->
		<link rel="stylesheet" href="<?php echo base_url('assets/css/hover-min.css')?>"/>

		<!--datepicker.css-->
		<link rel="stylesheet"  href="<?php echo base_url('assets/css/datepicker.css')?>"/>

		<!--owl.carousel.css-->
        <link rel="stylesheet" href="<?php echo base_url('assets/css/owl.carousel.min.css')?>"/>
		<link rel="stylesheet" href="<?php echo base_url('assets/css/owl.theme.default.min.css')?>"/>

		<!-- range css-->
        <link rel="stylesheet" href="<?php echo base_url('assets/css/jquery-ui.min.css')?>"/>

		<!--bootstrap.min.css-->
		<link rel="stylesheet" href="<?php echo base_url('assets/css/bootstrap.min.css')?>"/>

		<!-- bootsnav -->
		<link rel="stylesheet" href="<?php echo base_url('assets/css/bootsnav.css')?>"/>

		<!--style.css-->
		<link rel="stylesheet" href="<?php echo base_url('assets/css/style.css')?>"/>

		<!--responsive.css-->
		<link rel="stylesheet" href="<?php echo base_url('assets/css/responsive.css')?>"/>

		<!-- HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries -->
		<!-- WARNING: Respond.js doesn't work if you view the page via file:// -->

		<!--[if lt IE 9]>
		<script src="https://oss.maxcdn.com/html5shiv/3.7.3/html5shiv.min.js"></script>
		<script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
		<![endif]-->

	</head>
	<body>
		<!--[if lte IE 9]>
		<p class="browserupgrade">You are using an <strong>outdated</strong> browser. Please <a href="https://browsehappy.com/">upgrade
			your browser</a> to improve your experience and security.</p>
		<![endif]-->

		<!-- main-menu Start -->
		<header class="top-area">
			<div class="header-area">
				<div class="container">
					<div class="row">
						<div class="col-sm-2">
							<div class="logo">
								<a href="index.html">
									Wisata<span>Depok</span>
								</a>
							</div><!-- /.logo-->
						</div><!-- /.col-->
						<div class="col-sm-10">
							<div class="main-menu">
							
								<!-- Brand and toggle get grouped for better mobile display -->
								<div class="navbar-header">
									<button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">
										<i class="fa fa-bars"></i>
									</button><!-- / button-->
								</div><!-- /.navbar-header-->
								<div class="collapse navbar-collapse">		  
									<ul class="nav navbar-nav navbar-right">
										<li class="smooth-menu"><a href="#home">home</a></li>
										<li class="smooth-menu"><a href="#gallery">Tempat Wisata</a></li>
										<li class="smooth-menu"><a href="#pack">Rating </a></li>
										<li class="smooth-menu"><a href="#spo">Komentar</a></li>
										<li>
											<button class="book-btn">Log In
											</button>
										</li><!--/.project-btn--> 
									</ul>
								</div><!-- /.navbar-collapse -->
							</div><!-- /.main-menu-->
						</div><!-- /.col-->
					</div><!-- /.row -->
					<div class="home-border"></div><!-- /.home-border-->
				</div><!-- /.container-->
			</div><!-- /.header-area -->

		</header><!-- /.top-area-->
		<!-- main-menu End -->

		<!--about-us start -->
		<section id="home" class="about-us">
			<div class="container">
				<div class="about-us-content">
					<div class="row">
						<div class="col-sm-12">
							<div class="single-about-us">
								<div class="about-us-txt">
									<h2>
										Jelajahi Destinasi Indah di Depok

									</h2>
									<div class="about-btn">
										<button  class="about-view">
											cari
										</button>
									</div><!--/.about-btn-->
								</div><!--/.about-us-txt-->
							</div><!--/.single-about-us-->
						</div><!--/.col-->
						<div class="col-sm-0">
							<div class="single-about-us">
								
							</div><!--/.single-about-us-->
						</div><!--/.col-->
					</div><!--/.row-->
				</div><!--/.about-us-content-->
			</div><!--/.container-->

		</section><!--/.about-us-->
		<!--about-us end -->

		<!--travel-box start-->

		<!--galley start-->
		<section id="gallery" class="gallery">
			<div class="container">
				<div class="gallery-details">
					<div class="gallary-header text-center">
						<h2>
							Tempat Wisata Populer
						</h2>
						<p>
							Berikut beberapa tempat wisata paling populer di depok 
						</p>
					</div><!--/.gallery-header-->
					<div class="gallery-box">
						<div class="gallery-content">
						  	<div class="filtr-container">
						  		<div class="row">

						  			<div class="col-md-6">
						  				<div class="filtr-item">
											<img src="<?php echo base_url('assets/images/gallary/dkandang.jpg')?>" alt="portfolio image"/>
											<div class="item-title">
												<a href="#">
													D'Kandang Amazing Farm
												</a>
												<p><span>20780+ Pengunjung</span></p>
											</div><!-- /.item-title -->
										</div><!-- /.filtr-item -->
						  			</div><!-- /.col -->

						  			<div class="col-md-6">
						  				<div class="filtr-item">
											<img src="<?php echo base_url('assets/images/gallary/godong.jpg')?>" alt="portfolio image"/>
											<div class="item-title">
												<a href="#">
													Godong Ijo
												</a>
												<p><span>10974+ Pengunjung</span></p>
											</div> <!-- /.item-title-->
										</div><!-- /.filtr-item -->
						  			</div><!-- /.col -->

						  			<div class="col-md-4">
						  				<div class="filtr-item">
											<img src="<?php echo base_url('assets/images/gallary/mekarsari.jpg')?>" alt="portfolio image"/>
											<div class="item-title">
												<a href="#">
													Mekarsari
												</a>
												<p><span>30979+ Pengunjung</span></p>
											</div><!-- /.item-title -->
										</div><!-- /.filtr-item -->
						  			</div><!-- /.col -->

						  			<div class="col-md-4">
						  				<div class="filtr-item">
											<img src="<?php echo base_url('assets/images/gallary/kolam.jpg')?>" alt="portfolio image"/>
											<div class="item-title">
												<a href="#">
													Kolam Pasir putih 
												</a>
												<p><span>47890+ Pengunjung</span></p>
											</div> <!-- /.item-title-->
										</div><!-- /.filtr-item -->
						  			</div><!-- /.col -->

						  			<div class="col-md-4">
						  				<div class="filtr-item">
											<img src="<?php echo base_url('assets/images/gallary/studio.jpg')?>" alt="portfolio image"/>
											<div class="item-title">
												<a href="#">
													Studio Alam TVRI
												</a>
												<p><span>20957+ Pengunjung</span></p>
											</div> <!-- /.item-title-->
										</div><!-- /.filtr-item -->
						  			</div><!-- /.col -->

						  			<div class="col-md-8">
						  				<div class="filtr-item">
											<img src="<?php echo base_url('assets/images/gallary/Wiladatika.jpg')?>" alt="portfolio image"/>
											<div class="item-title">
												<a href="#">
													Taman Wiladatika
												</a>
												<p><span>50868+ Pengunjung</span></p>
											</div> <!-- /.item-title-->
										</div><!-- /.filtr-item -->
						  			</div><!-- /.col -->

						  		</div><!-- /.row -->

						  	</div><!-- /.filtr-container-->
						</div><!-- /.gallery-content -->
					</div><!--/.galley-box-->
				</div><!--/.gallery-details-->
			</div><!--/.container-->

		</section><!--/.gallery-->
		<!--gallery end-->

		<!--Rating Tempat-->
		<section id="pack" class="packages">
			<div class="container">
				<div class="gallary-header text-center">
					<h2>
						Rating Tempat 
					</h2>
					<p>
						Lihat  bagaimana tanggapan pengunjung di tempat wisata  
					</p>
				</div><!--/.gallery-header-->
				<div class="packages-content">
					<div class="row">
						<div class="col-md-4 col-sm-6">
							<div class="single-package-item">
								<img src="<?php echo base_url('assets/images/packages/wiladatika.jpg')?>" alt="package-place">
								<div class="single-package-item-txt">
									<h3>Taman wiladatika</h3>
									<div class="packages-para">
										<p>
											<span>
												<i class="fa fa-angle-right"></i> 08:00 - 16:00 WIB
											</span>
											<i class="fa fa-angle-right"></i> Taman Bunga
										</p>
										<p>
											<span>
												<i class="fa fa-angle-right"></i> Playground
											</span>
											<i class="fa fa-angle-right"></i> Outbond
										 </p>
									</div><!--/.packages-para-->
									<div class="packages-review">
										<p>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<span>104 review</span>
										</p>
									</div><!--/.packages-review-->
									<div class="about-btn">
										<button  class="about-view packages-btn">
											Komentar
										</button>
									</div><!--/.about-btn-->
								</div><!--/.single-package-item-txt-->
							</div><!--/.single-package-item-->

						</div><!--/.col-->
						<div class="col-md-4 col-sm-6">
							<div class="single-package-item">
								<img src="<?php echo base_url('assets/images/packages/dkandang.jpg')?>" alt="package-place">
								<div class="single-package-item-txt">
									<h3>D'Kandang Amazing Farm</h3>
									<div class="packages-para">
										<p>
											<span>
												<i class="fa fa-angle-right"></i> 09:00 - 16:00 WIB
											</span>
											<i class="fa fa-angle-right"></i> Gazebo
										</p>
										<p>
											<span>
												<i class="fa fa-angle-right"></i> Outbond
											</span>
											<i class="fa fa-angle-right"></i> Peternakan
										 </p>
									</div><!--/.packages-para-->
									<div class="packages-review">
										<p>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<span>133 review</span>
										</p>
									</div><!--/.packages-review-->
									<div class="about-btn">
										<button  class="about-view packages-btn">
											Komentar
										</button>
									</div><!--/.about-btn-->
								</div><!--/.single-package-item-txt-->
							</div><!--/.single-package-item-->

						</div><!--/.col-->
						
						<div class="col-md-4 col-sm-6">
							<div class="single-package-item">
								<img src="<?php echo base_url('assets/images/packages/mekarsari.jpg')?>" alt="package-place">
								<div class="single-package-item-txt">
									<h3>Mekarsari</h3>
									<div class="packages-para">
										<p>
											<span>
												<i class="fa fa-angle-right"></i> 09:00 - 15:00 WIB
											</span>
											<i class="fa fa-angle-right"></i> Kids Outbond
										</p>
										<p>
											<span>
												<i class="fa fa-angle-right"></i> Taman Luas
											</span>
											<i class="fa fa-angle-right"></i>  Kereta wisata
										 </p>
									</div><!--/.packages-para-->
									<div class="packages-review">
										<p>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<span>217 review</span>
										</p>
									</div><!--/.packages-review-->
									<div class="about-btn">
										<button  class="about-view packages-btn">
											Komentar
										</button>
									</div><!--/.about-btn-->
								</div><!--/.single-package-item-txt-->
							</div><!--/.single-package-item-->

						</div><!--/.col-->
						
						<div class="col-md-4 col-sm-6">
							<div class="single-package-item">
								<img src="<?php echo base_url('assets/images/packages/godong.jpg')?>" alt="package-place">
								<div class="single-package-item-txt">
									<h3>Godong Ijo</h3>
									<div class="packages-para">
										<p>
											<span>
												<i class="fa fa-angle-right"></i> 09:00 - 20:00 WIB
											</span>
											<i class="fa fa-angle-right"></i> Aula Dome
										</p>
										<p>
											<span>
												<i class="fa fa-angle-right"></i> Cafe & Resto
											</span>
											<i class="fa fa-angle-right"></i> Pemancingan
										 </p>
									</div><!--/.packages-para-->
									<div class="packages-review">
										<p>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<span>166 review</span>
										</p>
									</div><!--/.packages-review-->
									<div class="about-btn">
										<button  class="about-view packages-btn">
											Komentar
										</button>
									</div><!--/.about-btn-->
								</div><!--/.single-package-item-txt-->
							</div><!--/.single-package-item-->

						</div><!--/.col-->
						
						<div class="col-md-4 col-sm-6">
							<div class="single-package-item">
								<img src="<?php echo base_url('assets/images/packages/kolam.jpg')?>" alt="package-place">
								<div class="single-package-item-txt">
									<h3>Kolam Pasir Putih</h3>
									<div class="packages-para">
										<p>
											<span>
												<i class="fa fa-angle-right"></i> 09:00 - 17:00 WIB
											</span>
											<i class="fa fa-angle-right"></i> Bungalow
										</p>
										<p>
											<span>
												<i class="fa fa-angle-right"></i> Playground
											</span>
											<i class="fa fa-angle-right"></i> pemancingan
										 </p>
									</div><!--/.packages-para-->
									<div class="packages-review">
										<p>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<span>300 review</span>
										</p>
									</div><!--/.packages-review-->
									<div class="about-btn">
										<button  class="about-view packages-btn">
											Komentar
										</button>
									</div><!--/.about-btn-->
								</div><!--/.single-package-item-txt-->
							</div><!--/.single-package-item-->

						</div><!--/.col-->
						
						<div class="col-md-4 col-sm-6">
							<div class="single-package-item">
								<img src="<?php echo base_url('assets/images/packages/studio.jpg')?>" alt="package-place">
								<div class="single-package-item-txt">
									<h3>Studio Alam TVRI</h3>
									<div class="packages-para">
										<p>
											<span>
												<i class="fa fa-angle-right"></i> 08:00 - 16:00 WIB
											</span>
											<i class="fa fa-angle-right"></i> Area luas
										</p>
										<p>
											<span>
												<i class="fa fa-angle-right"></i> cocok untuk gathering
											</span>
											<i class="fa fa-angle-right"></i> food facilities
										 </p>
									</div><!--/.packages-para-->
									<div class="packages-review">
										<p>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<i class="fa fa-star"></i>
											<span>144 review</span>
										</p>
									</div><!--/.packages-review-->
									<div class="about-btn">
										<button  class="about-view packages-btn">
											Komentar
										</button>
									</div><!--/.about-btn-->
								</div><!--/.single-package-item-txt-->
							</div><!--/.single-package-item-->

						</div><!--/.col-->
					</div><!--/.row-->
				</div><!--/.packages-content-->
			</div><!--/.container-->

		</section><!--/.packages-->
		<!--packages end-->

		<!-- komentar Start -->
		<section   class="testemonial">
			<div class="container">

				<div class="gallary-header text-center">
					<h2>
						Komentar Pengunjung
					</h2>
					<p>
						Beberapa komentar pengunjung yang dapat dijadikan referensi
					</p>

				</div><!--/.gallery-header-->

				<div class="owl-carousel owl-theme" id="testemonial-carousel">

					<div class="home1-testm item">
						<div class="home1-testm-single text-center">
							<div class="home1-testm-img">
								<img src="<?php echo base_url('assets/images/client/p11.jpg')?>" alt="img"/>
							</div><!--/.home1-testm-img-->
							<div class="home1-testm-txt">
								<span class="icon section-icon">
									<i class="fa fa-quote-left" aria-hidden="true"></i>
								</span>
								<p>
									Ajak anak anak ketempat ini seruu, bisa berinteraksi dengan hewan yang ada di peternakan, seperti memeras susu sapi.
								</p>
								<h3>
									<a href="#">
										fajar maulana
									</a>
								</h3>
								<h4>D'kandang Amazing Farm</h4>
							</div><!--/.home1-testm-txt-->	
						</div><!--/.home1-testm-single-->

					</div><!--/.item-->

					<div class="home1-testm item">
						<div class="home1-testm-single text-center">
							<div class="home1-testm-img">
								<img src="<?php echo base_url('assets/images/client/p22.jpg')?>" alt="img"/>
							</div><!--/.home1-testm-img-->
							<div class="home1-testm-txt">
								<span class="icon section-icon">
									<i class="fa fa-quote-left" aria-hidden="true"></i>
								</span>
								<p>
									areanya luas, Outbond untuk Anak - anak tidak membosankan. Ada playground kecil kecilan juga bagi yang tidak niat outbond.
								</p>
								<h3>
									<a href="#">
										Ahmad wijaya
									</a>
								</h3>
								<h4>Taman wiladatika</h4>
							</div><!--/.home1-testm-txt-->	
						</div><!--/.home1-testm-single-->

					</div><!--/.item-->

					<div class="home1-testm item">
						<div class="home1-testm-single text-center">
							<div class="home1-testm-img">
								<img src="<?php echo base_url('assets/images/client/p33.jpg')?>" alt="img"/>
							</div><!--/.home1-testm-img-->
							<div class="home1-testm-txt">
								<span class="icon section-icon">
									<i class="fa fa-quote-left" aria-hidden="true"></i>
								</span>
								<p>
									Setelah ditutup karna covid, sekarang tempatnya lebih bagus, banyak promo menarik jika mengajak rombongan kesana.
								</p>
								<h3>
									<a href="#">
										Marina
									</a>
								</h3>
								<h4>Mekarsari</h4>
							</div><!--/.home1-testm-txt-->	
						</div><!--/.home1-testm-single-->

					</div><!--/.item-->

					<div class="home1-testm item">
						<div class="home1-testm-single text-center">
							<div class="home1-testm-img">
								<img src="<?php echo base_url('assets/images/client/p4.jpg')?>" alt="img"/>
							</div><!--/.home1-testm-img-->
							<div class="home1-testm-txt">
								<span class="icon section-icon">
									<i class="fa fa-quote-left" aria-hidden="true"></i>
								</span>
								<p>
									Makanan disini enak enak.tempatnya luas, disini juga ada pemancingan, jadi orangtua dan anak sama sama senang. aula nya bisa dipakai untuk acara keluarga
								</p>
								<h3>
									<a href="#">
										ibrahim
									</a>
								</h3>
								<h4>Godong Ijo</h4>
							</div><!--/.home1-testm-txt-->	
						</div><!--/.home1-testm-single-->

					</div><!--/.item-->

					<div class="home1-testm item">
						<div class="home1-testm-single text-center">
							<div class="home1-testm-img">
							<img src="<?php echo base_url('assets/images/client/p5.jpg')?>" alt="img"/>
							</div><!--/.home1-testm-img-->
							<div class="home1-testm-txt">
								<span class="icon section-icon">
									<i class="fa fa-quote-left" aria-hidden="true"></i>
								</span>
								<p>
									Wahana permainan airnya banyak. toilet disini juga bersih. penjaga kolam sangat sigap. semoga selalu bagus seperti ini
								</p>
								<h3>
									<a href="#">
										indah permata
									</a>
								</h3>
								<h4>Pasir Putih</h4>
							</div><!--/.home1-testm-txt-->	
						</div><!--/.home1-testm-single-->

					</div><!--/.item-->

					<div class="home1-testm item">
						<div class="home1-testm-single text-center">
							<div class="home1-testm-img">
								<img src="<?php echo base_url('assets/images/client/p6.jpeg')?>" alt="img"/>
							</div><!--/.home1-testm-img-->
							<div class="home1-testm-txt">
								<span class="icon section-icon">
									<i class="fa fa-quote-left" aria-hidden="true"></i>
								</span>
								<p>
									Tempatnya dekat dengan rumah. cocok untuk bersantai dengan keluarga, karna tempatnya yang luas,rindang, dan banyak makanan makanan yang dijual.
								</p>
								<h3>
									<a href="#">
										bayu
									</a>
								</h3>
								<h4>Studio Alam TVRI</h4>
							</div><!--/.home1-testm-txt-->	
						</div><!--/.home1-testm-single-->

					</div><!--/.item-->
				</div><!--/.testemonial-carousel-->
			</div><!--/.container-->

		</section><!--/.testimonial-->	
		<!-- testemonial End -->


		<!-- footer-copyright start -->
		<footer  class="footer-copyright">
			<div class="container">
				<div class="footer-content">
					<div class="row">

						<div class="col-sm-3">
							<div class="single-footer-item">
								<div class="footer-logo">
									<a href="index.html">
										Wisata<span>Depok</span>
									</a>
									<p>
										tempat wisata terbaik di depok
									</p>
								</div>
							</div><!--/.single-footer-item-->
						</div><!--/.col-->

						<div class="col-sm-3">
							<div class="single-footer-item">
								<h2>link</h2>
								<div class="single-footer-txt">
									<p><a href="#">home</a></p>
									<p><a href="#">tempat wisata</a></p>
									<p><a href="#">rating</a></p>
									<p><a href="#">komentar</a></p>
									<p><a href="#">contacts</a></p>
								</div><!--/.single-footer-txt-->
							</div><!--/.single-footer-item-->

						</div><!--/.col-->

						<div class="col-sm-3">
							<div class="single-footer-item">
								<h2>Tempat Wisata Populer</h2>
								<div class="single-footer-txt">
									<p><a href="#">Taman Wiladatika</a></p>
									<p><a href="#">D'Kandang Amazing Farm</a></p>
									<p><a href="#">Mekarsari</a></p>
									<p><a href="#">Godong Ijo</a></p>
									<p><a href="#">Kolam Pasir Putih</a></p>
									<p><a href="#">Studio Alam TVRI</a></p>
								</div><!--/.single-footer-txt-->
							</div><!--/.single-footer-item-->
						</div><!--/.col-->

						<div class="col-sm-3">
							<div class="single-footer-item text-center">
								<h2 class="text-left">kontak</h2>
								<div class="single-footer-txt text-left">
									<p>+62 8267890172</p>
									<p class="foot-email"><a href="#">info@wisatadepok.com</a></p>
									<p>Depok, Indonesia</p>
								</div><!--/.single-footer-txt-->
							</div><!--/.single-footer-item-->
						</div><!--/.col-->

					</div><!--/.row-->

				</div><!--/.footer-content-->
				<hr>
				<div class="foot-icons ">
					<ul class="footer-social-links list-inline list-unstyled">
		                <li><a href="#" target="_blank" class="foot-icon-bg-1"><i class="fa fa-facebook"></i></a></li>
		                <li><a href="#" target="_blank" class="foot-icon-bg-2"><i class="fa fa-twitter"></i></a></li>
		                <li><a href="#" target="_blank" class="foot-icon-bg-3"><i class="fa fa-instagram"></i></a></li>
		        	</ul>
		        	<p>&copy; 2017 <a href="<?php echo base_url('https://www.themesine.com')?>">ThemeSINE</a>. All Right Reserved</p>

		        </div><!--/.foot-icons-->
				<div id="scroll-Top">
					<i class="fa fa-angle-double-up return-to-top" id="scroll-top" data-toggle="tooltip" data-placement="top" title="" data-original-title="Back to Top" aria-hidden="true"></i>
				</div><!--/.scroll-Top-->
			</div><!-- /.container-->

		</footer><!-- /.footer-copyright-->
		<!-- footer-copyright end -->




		<script src="<?php echo base_url('assets/js/jquery.js')?>"></script>
		<!-- Include all compiled plugins (below), or include individual files as needed -->

		<!--modernizr.min.js-->
		<script  src="<?php echo base_url('https://cdnjs.cloudflare.com/ajax/libs/modernizr/2.8.3/modernizr.min.js"')?>"></script>


		<!--bootstrap.min.js-->
		<script  src="<?php echo base_url('assets/js/bootstrap.min.js')?>"></script>

		<!-- bootsnav js -->
		<script src="<?php echo base_url('assets/js/bootsnav.js')?>"></script>

		<!-- jquery.filterizr.min.js -->
		<script src="<?php echo base_url('assets/js/jquery.filterizr.min.js')?>"></script>

		<script  src="<?php echo base_url('https://cdnjs.cloudflare.com/ajax/libs/jquery-easing/1.4.1/jquery.easing.min.js')?>"></script>

		<!--jquery-ui.min.js-->
        <script src="<?php echo base_url('assets/js/jquery-ui.min.js')?>"></script>

        <!-- counter js -->
		<script src="<?php echo base_url('assets/js/jquery.counterup.min.js')?>"></script>
		<script src="<?php echo base_url('assets/js/waypoints.min.js')?>"></script>

		<!--owl.carousel.js-->
        <script  src="<?php echo base_url('assets/js/owl.carousel.min.js')?>"></script>

        <!-- jquery.sticky.js -->
		<script src="<?php echo base_url('assets/js/jquery.sticky.js')?>"></script>

        <!--datepicker.js-->
        <script  src="<?php echo base_url('assets/js/datepicker.js')?>"></script>

		<!--Custom JS-->
		<script src="<?php echo base_url('assets/js/custom.js')?>"></script>


	</body>

</html>